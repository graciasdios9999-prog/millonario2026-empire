...
async function run() {
  ...
  test('buildProductPayload creates SEO + price + tags', () => {
    const payload = productEngine.buildProductPayload({
      title: 'Guia Reset 40+', tier: 'CORE', benefits: ['Plan 30 dias', 'Checklist'],
    });
  ...
  const input = {
    title: 'Test Product Idem', tier: 'ENTRY', dryRun: true, idempotencyKey: 'test-key-001',
  };
  ...
  const order = { id: 999001, total_price: '49.00', currency: 'USD', financial_status: 'paid' };
