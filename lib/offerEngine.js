/**
 * Offer engine — scoring + funnel architecture (no fake revenue).
 */
'use strict';
...
function scoreProduct(input) {
  ...
  const recommendation = publishReady
    ? 'PUBLISH_CANDIDATE'
    : finalScore >= 40
      ? 'IMPROVE_OFFER'
      : 'SKIP';
  ...
  logger.info('offerEngine.scoreProduct', 'SUCCESS', {
    score: finalScore,
    recommendation,
    tier: result.tier,
  });
...
function buildOfferStack(core) {
  ...
  return {
    ENTRY: {
      role: 'low-friction entry',
      title: title + ' — Starter',
      suggestedPrice: Math.max(7, Math.round(basePrice * 0.33 * 100) / 100),
      tier: 'ENTRY',
    },
    ...
    PREMIUM: {
      role: 'high-ticket',
      title: title + ' — Premium',
      suggestedPrice: Math.round(basePrice * 3.2 * 100) / 100,
      tier: 'PREMIUM',
    },
    BUNDLE: {
      role: 'increase AOV',
      title: title + ' + Bonuses Bundle',
      suggestedPrice: Math.round(basePrice * 1.6 * 100) / 100,
      tier: 'BUNDLE',
    },
    UPSELL: {
      role: 'post-purchase / checkout upsell',
      title: 'Add-on for ' + title,
      suggestedPrice: Math.round(basePrice * 0.55 * 100) / 100,
      tier: 'UPSELL',
    },
    CROSS_SELL: {
      role: 'related attach',
      title: 'Related for ' + title,
      suggestedPrice: Math.round(basePrice * 0.4 * 100) / 100,
      tier: 'CROSS_SELL',
    },
    funnel: FUNNEL,
...
module.exports = { scoreProduct, buildOfferStack, FUNNEL };
