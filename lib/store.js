...
function getProductByKey(idempotencyKey) {
  const state = load();
  return state.products[idempotencyKey] || null;
}

function saveProduct(idempotencyKey, productRecord) {
  const state = load();
  state.products[idempotencyKey] = {
    ...productRecord,
...
function recordVerifiedOrder(order) {
  ...
  if (state.orders_verified[id]) {
    return { duplicate: true, order: state.orders_verified[id] };
  }
  const record = {
    id,
